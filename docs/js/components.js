    ...
    <header class="fixed top-0 z-50 w-full backdrop-blur transition-colors duration-500 border-b border-slate-900/10 dark:border-slate-50/[0.06] bg-white/95 dark:bg-slate-900/75" style="background-color: rgba(255,255,255,0.95);">
        <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
            <div class="flex items-center gap-4">
                <button id="mobile-menu-btn" class="lg:hidden text-slate-500 hover:text-slate-600 dark:text-slate-400 dark:hover:text-slate-300">
            ...
            <div class="flex items-center gap-4">
                ...
                <button id="search-btn" class="flex items-center gap-2 px-3 py-1.5 text-sm text-slate-400 border border-slate-200 dark:border-slate-700 rounded-lg hover:border-blue-500 hover:text-blue-500 transition-colors" title="Search (⌘K)">
                ...
                <div class="flex items-center gap-3 border-l pl-4 border-slate-200 dark:border-slate-800">
                    ...
                    <button id="theme-toggle" class="text-slate-400 hover:text-slate-500 dark:hover:text-slate-300 transition-colors" title="Toggle theme">
                        <svg id="theme-icon-dark" class="w-5 h-5 hidden dark:block" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
                        <svg id="theme-icon-light" class="w-5 h-5 dark:hidden" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"></path></svg>
                    </button>
                </div>
            </div>
        </div>
    ...
    <footer class="py-12 border-t border-slate-200 dark:border-slate-800" style="background: var(--bg-color);">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mt-8 pt-8 border-t border-slate-200 dark:border-slate-800 text-center text-sm text-slate-400 dark:text-slate-500">
                <p>Built with ❤️ by Truong Quang Tuan • v${SITE.version} • ${SITE.iosTarget}</p>
            </div>
        </div>
    </footer>`;
}
    ...
    setTimeout(() => {
        ...
        if (themeBtn) {
            themeBtn.addEventListener('click', () => {
                ...
                // Re-render header with correct dark mode background
